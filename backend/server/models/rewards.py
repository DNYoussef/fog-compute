...
class RewardDistribution(Base):
    ...
    reward_id = Column(String(255), nullable=False, unique=True, index=True)
    ...
    reward_type = Column(String(50), nullable=False)  # staking, deployment_runtime, task_completion, etc.
    ...
    transfer_tx_id = Column(String(255), nullable=True)
    rollback_tx_id = Column(String(255), nullable=True)
...
class RewardDistributionBatch(Base):
    ...
    # Batch metadata
    batch_type = Column(String(50), nullable=False)  # deployment_cleanup, system_cleanup, manual
    trigger_event = Column(String(255), nullable=True)  # deployment_deletion, service_shutdown, etc.
    deployment_id = Column(UUID(as_uuid=True), ForeignKey('deployments.id'), nullable=True, index=True)
    ...
    # Batch statistics
    total_rewards = Column(Float, default=0, nullable=False)
    total_amount = Column(Float, default=0.0, nullable=False)
    successful_distributions = Column(Float, default=0, nullable=False)
    failed_distributions = Column(Float, default=0, nullable=False)
...
class RewardDistributionAuditLog(Base):
    ...
    reward_id = Column(String(255), nullable=False, index=True)

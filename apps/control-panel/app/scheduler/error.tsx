...
export default function SchedulerError({ error, reset }: ErrorProps) {
  useEffect(() => {
    console.error('Scheduler page error:', error);
  }, [error]);

  return (
    <div className="flex items-center justify-center min-h-[60vh]" data-testid="scheduler-error">
      <div className="glass rounded-xl p-8 max-w-xl mx-4">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h3 className="text-xl font-semibold text-red-400">Scheduler Error</h3>
            <p className="text-gray-400 text-sm">
              Failed to load batch job scheduler
            </p>
          </div>
        </div>
